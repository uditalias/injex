...
class InjexContainer {
    ...
    async bootstrap() {
        ...
        if (this.bootstrapModuleName) {
            const bootstrap = this.get(this.bootstrapModuleName);
            await bootstrap.run();
        }
        return this;
    }
    ...
    register(item) {
        ...
        if (metadata.bootstrap) {
            if (this.bootstrapModuleName) {
                throw new errors_1.MultipleBootstrapModulesFound(this.bootstrapModuleName, metadata.name);
            }
            this.bootstrapModuleName = metadata.name;
        }
